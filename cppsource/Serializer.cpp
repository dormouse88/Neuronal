...
{
    //doc.remove_child("PLAN"); //(How to delete all plans)
...
std::shared_ptr<ChipPlan> Serializer::LoadNode(pugi::xml_node & doc, BlobFactory & factory_p, int planID)
{
    ...
    //can't use std::to_string due to MinGW bug...
    std::ostringstream stm ;
    stm << planID;
    pugi::xml_node xmlPlan = doc.find_child_by_attribute("PLAN", "i", stm.str().c_str());
